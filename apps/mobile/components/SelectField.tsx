  ...
  return (
    <View className="gap-1">
      ...
      <Pressable
        ...
        className="rounded-brand-md border border-brand-border bg-brand-surface px-3 py-2.5"
      >
        <Text
          className={selected ? "text-base text-brand-text" : "text-base text-brand-text-muted"}
        >
      ...
      <Modal visible={open} animationType="slide" transparent onRequestClose={() => setOpen(false)}>
        ...
        <SafeAreaView className="max-h-[70%] rounded-t-brand-lg bg-brand-surface">
          <View className="flex-row items-center justify-between border-b border-brand-border px-4 py-3">
            <Text className="text-base font-semibold text-brand-primary-dark">{label}</Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                ...
                className="border-b border-brand-border px-4 py-3"
              >
                <Text
                  className={
                    item.value === value
