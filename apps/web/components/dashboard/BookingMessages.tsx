...
// Zwei-Wege-Nachrichten-Thread mit der Familie - siehe BookingMessage in
// schema.prisma. Anders als ResidentNotes (intern, nur fürs Heim) sieht die
// Familie hier mit, in ihrem eigenen Konto unter "Meine Buchungen" (siehe
// apps/web/components/account/BookingMessagesThread.tsx für die Gegenseite,
// gleiches UI-Muster, andere tRPC-Prozeduren).
export function BookingMessages({ bookingId }: { bookingId: string }) {
  const utils = trpc.useUtils();
  const messages = trpc.operator.bookingMessages.useQuery({ bookingId });
  const [body, setBody] = useState("");
  const [error, setError] = useState<string | null>(null);

  const markRead = trpc.operator.markBookingMessagesRead.useMutation();
  const sendMessage = trpc.operator.sendBookingMessage.useMutation({
    onSuccess: () => {
      setBody("");
      void utils.operator.bookingMessages.invalidate({ bookingId });
    },
  ...
  return (
    <div className="flex flex-col gap-3 rounded-brand-md border border-brand-border bg-brand-background p-4">
      <p className="text-xs text-brand-text-muted">
        Nachrichten an die buchende Person - sie sieht sie in ihrem woodaa-Konto und bekommt eine
          ...
          {messages.data.map((message) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-brand-text-muted">
                {message.senderType === "FACILITY" ? "Ihr" : "Familie"} ·{" "}
